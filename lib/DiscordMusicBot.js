...
} = require("../util/managedUpdate");
const {
  createHerokuBuild,
  getHerokuBuild,
  getHerokuDeployConfig,
} = require("../util/herokuDeploy");
const {
  PROGRESS_SEGMENTS,
...
class DiscordMusicBot extends Client {
  ...
  constructor(
    ...
    ConfigFetcher()
      .then(async (conf) => {
        ...
        if (this.config.publicStatusApi?.enabled === true) {
          this.publicStatusServer = require("../api/public-status-server")(
            this
          );
        }
        this.build();
    ...
    this.commandsRan = 0;
    this.songsPlayed = 0;
    // Used to ensure that an old Heroku dyno never restores players while a
    // newly released dyno is still replacing it.
  ...
  getManagedUpdateKey() {
    return "runtime:managed_bot_update";
  }

  getManagedUpdateMode() {
    return getManagedUpdateMode();
  }

  isHerokuDeployAvailable() {
    return Boolean(getHerokuDeployConfig(this.config));
  }

  async queueManagedBotUpdate({ user, channelId, guildId }) {
    if (!this.database) throw new Error("Database is not ready.");
    const mode = this.getManagedUpdateMode();
    if (mode === "unavailable") {
      const error = new Error("Managed update helper is unavailable.");
      error.code = "MANAGED_UPDATE_UNAVAILABLE";
      throw error;
    }
    if (mode === "heroku" && !this.isHerokuDeployAvailable()) {
      const error = new Error("Heroku deployment is not configured.");
      error.code = "HEROKU_DEPLOY_UNAVAILABLE";
      throw error;
    }

    const previous = await this.database.get(this.getManagedUpdateKey());
    if (
      [
        "queued",
        "restoring",
        "heroku_building",
        "heroku_release_pending",
      ].includes(
        previous?.state
      )
    ) {
      const error = new Error("A bot restart is already in progress.");
      error.code = "MANAGED_UPDATE_IN_PROGRESS";
      throw error;
    }

    const updateId = randomUUID();
    const snapshots = createPlayerRecoverySnapshots(this, [
      ...this.manager.players.values(),
    ]);
    const journal = {
      id: updateId,
      mode,
      state: mode === "heroku" ? "heroku_building" : "queued",
      requestedAt: new Date().toISOString(),
      initiatingInstanceId: this.runtimeInstanceId,
      requesterId: user?.id || null,
      requesterTag: user?.tag || user?.username || null,
      channelId: channelId || null,
      guildId: guildId || null,
      snapshots,
    };

    await this.database.set(this.getManagedUpdateKey(), journal);
    if (mode === "managed") {
      try {
        requestManagedUpdate({
          id: updateId,
        ...
        await this.database.set(this.getManagedUpdateKey(), {
          ...journal,
          state: "failed",
      ...
      this.startManagedUpdateMonitor();
    }
    if (mode === "heroku") {
      try {
        const build = await createHerokuBuild(this.config);
        const queuedJournal = {
          ...journal,
          herokuBuildId: build.id,
          commit: build.commit,
        };
        await this.database.set(this.getManagedUpdateKey(), queuedJournal);
        this.startManagedUpdateMonitor();
        return queuedJournal;
      } catch (error) {
        await this.database.set(this.getManagedUpdateKey(), {
          ...journal,
          state: "failed",
  ...
  async sendManagedUpdateStatus(journal, color, messageKey, vars = {}) {
    if (!journal?.channelId) return;

    let channel = this.channels.cache.get(journal.channelId);
    if (!channel) {
      channel = await this.channels.fetch(journal.channelId).catch(() => null);
    }
      ...
      .send({
        embeds: [
          new EmbedBuilder()
            .setColor(color)
            .setDescription(
              this.translateGuild(journal.guildId, messageKey, vars)
            )
            .setTimestamp(),
        ],
      })
  ...
  startManagedUpdateMonitor() {
    if (this.managedUpdateMonitor) return;

    this.managedUpdateMonitor = setInterval(() => {
      this.resumeManagedBotUpdate().catch((error) => {
        this.warn(`Managed update monitor error: ${error.message}`);
      });
    }, 5_000);
    this.managedUpdateMonitor.unref?.();
  }

  stopManagedUpdateMonitor() {
    if (!this.managedUpdateMonitor) return;
    clearInterval(this.managedUpdateMonitor);
    this.managedUpdateMonitor = null;
  }

  async resumeManagedBotUpdate() {
    if (!this.database || this.managedUpdateRecoveryInProgress) return;

    const journal = await this.database.get(this.getManagedUpdateKey());
    if (
      !journal ||
      ![
        "queued",
      ...
      this.stopManagedUpdateMonitor();
      return;
    }

    let result = null;
    if (journal.mode === "heroku") {
      result = await getHerokuBuild(this.config, journal.herokuBuildId);
      if (result.state === "running") {
        this.startManagedUpdateMonitor();
        return;
      }

      // A successful Build API request creates a release automatically.  The
      // old dyno can still be alive for a few seconds, so it must leave the
      // journal for the replacement dyno instead of restoring duplicate
      // players itself.
      if (
        result.state === "success" &&
        journal.initiatingInstanceId === this.runtimeInstanceId
      ) {
        await this.database.set(this.getManagedUpdateKey(), {
          ...journal,
          state: "heroku_release_pending",
          buildSucceededAt: new Date().toISOString(),
          commit: result.commit || journal.commit || null,
        });
        this.stopManagedUpdateMonitor();
        return;
      }
    } else {
      result = readManagedUpdateResult();
      if (!result || result.id !== journal.id || result.state === "running") {
        this.startManagedUpdateMonitor();
        return;
      }
    }
    ...
    if (result.state !== "success") {
      const updaterError =
        result.error ||
        this.translateGuild(journal.guildId, "reload.unknownUpdaterError");
      await this.sendManagedUpdateStatus(
        journal,
        "#FF0000",
        "reload.hardUpdateFailed",
        { error: updaterError }
      );
      await this.database.set(this.getManagedUpdateKey(), {
        ...journal,
    ...
    this.managedUpdateRecoveryInProgress = true;
    try {
      await this.database.set(this.getManagedUpdateKey(), {
        ...journal,
        state: "restoring",
      ...
      const recoverable = (journal.snapshots || []).length;
      let node = null;
      if (recoverable > 0) {
        node = await waitForAvailableLavalink(this);
        if (!node) {
          throw new Error(
            this.translateGuild(
              journal.guildId,
              "reload.noLavalinkAfterRestart"
            )
          );
        }
      }
      ...
      for (const snapshot of journal.snapshots || []) {
        try {
          await restoreRecoveredPlayer(this, snapshot, node);
          restored += 1;
          await sendRecoveryNotice(this, snapshot, "error.musicRestored");
        } catch (error) {
          this.warn(
            `Could not restore player for guild ${snapshot.guildId}: ${error.message}`
          );
          await sendRecoveryNotice(this, snapshot, "error.musicRestoreFailed");
        }
      }
      ...
      this.managedUpdateRecoveryInProgress = false;
      this.stopManagedUpdateMonitor();
    }
  }
  ...
  async saveLavalinkNodes(nodes) {
    ...
    if (!Array.isArray(nodes))
      throw new TypeError("Lavalink nodes must be an array.");
  ...
  async applyPersistedRuntimeConfig() {
    ...
    if (
      persistedPresence &&
      typeof persistedPresence === "object" &&
      !Array.isArray(persistedPresence)
    ) {
  ...
  async getGuildLanguage(guildId) {
    ...
    try {
      language = normalizeLanguage(
        await this.database.get(this.getGuildLanguageKey(guildId))
      );
    } catch {
  ...
  runWithGuildLanguage(guildId, callback) {
    return runWithLanguage(
      this.guildLanguages.get(guildId) || DEFAULT_LANGUAGE,
      callback
    );
  }
  ...
  build() {
    ...
    client.sendLavalinkNotification = async (embed, nodeId = null) => {
      try {
        ...
        if (logChannel) {
          ...
          client.warn(
            t("system.lavalinkNotifyNoChannel", {
    ...
    this.manager.nodeManager
      .on("connect", (node) => {
        ...
        this.log(
          t("system.lavalinkConnected", { id: node.id, host: h, port: p })
        );
        ...
        sendLavalinkNotification(
          new EmbedBuilder()
            .setColor("#00FF00")
            .setDescription(
              t("lavalink.lavalinkConnectedNotify", {
                id: node.id,
                host: h,
                port: p,
                ssl: s,
      ...
      .on("disconnect", (node) => {
        ...
        const s = node.options?.secure ? "True" : "False";
        this.warn(
          t("system.lavalinkDisconnected", { id: node.id, host: h, port: p })
        );
        ...
        let failoverMsg = t("DiscordMusicBot.auto_303", {
          var1: node.id,
          var2: h,
          var3: p,
          var4: s,
        ...
        if (affectedPlayers.length > 0)
          failoverMsg += t("DiscordMusicBot.auto_304", {
            var1: affectedPlayers.length,
          });
        if (aliveNodes.length > 0) {
          failoverMsg += t("DiscordMusicBot.auto_305", {
            var1: aliveNodes.map((n) => `\`${n.id}\``).join(", "),
          });
      ...
      .on("error", (node, err) => {
        this.warn(
          t("system.lavalinkError", { id: node.id, error: err.message })
        );
        ...
        sendLavalinkNotification(
          new EmbedBuilder()
            .setColor("#FF8800")
            .setDescription(
              t("lavalink.lavalinkErrorNotify", {
                id: node.id,
                error: err.message,
              })
            )
    ...
    this.manager
      .on("trackError", (player, track, payload) => {
        ...
        let errorEmbed = new EmbedBuilder()
          .setColor(0xff0000)
          .setTitle(
            client.translateGuild(player.guildId, "error.trackErrorTitle")
          )
          .setDescription(
            client.translateGuild(player.guildId, "error.trackErrorDesc", {
              title,
            })
          )
          .setFooter({
            text: client.translateGuild(
              player.guildId,
              "error.trackErrorFooter"
            ),
          });
      ...
      .on("trackStuck", (player, track) => {
        ...
        let errorEmbed = new EmbedBuilder()
          .setColor(0xff0000)
          .setTitle(
            client.translateGuild(player.guildId, "error.trackStuckTitle")
          )
          .setDescription(
            client.translateGuild(player.guildId, "error.trackErrorDesc", {
              title,
      ...
      .on("playerCreate", (player) => {
        ...
        client.applyGuildAutoSettingsToPlayer(player).catch((error) => {
          client.warn(
            `Could not load automatic settings for ${player.guildId}: ${error.message}`
          );
        });
        ...
        this.warn(
          t("DiscordMusicBot.auto_309", {
            var1: player.guildId,
            var2: client.guilds.cache.get(player.guildId)
      ...
      .on("trackStart", async (player, track) => {
        ...
        client.setVoiceStatus(
          player.voiceChannelId,
          `🎵 ${track.info.title}`.substring(0, 175)
        );
        this.songsPlayed++;
        ...
        if (!forceNew && existingMsg && !client.isMessageDeleted(existingMsg)) {
          try {
            nowPlaying = await queueNowPlayingMessageEdit(
              player,
              existingMsg,
              messagePayload
            );
        ...
        if (!autoQueue || !tracksAdded) {
          const showQueueEndAndDisconnect = async () => {
            ...
            try {
              if (
                !player.playing &&
                getHumanMemberCount(getVoiceChannel(client, player)) === 0
              ) {
                ...
                const disconnectTimeout = setTimeout(async () => {
                  if (
                    !player.playing &&
                    player.connected &&
                    !player.get("twentyFourSeven")
                  ) {
                    let disconnectedEmbed = new EmbedBuilder()
                      .setColor(client.config.embedColor)
                      .setAuthor({
                        name: client.translateGuild(
                          player.guildId,
                          "voice.disconnected"
                        ),
                      })
    ...
    fs.readdir(SlashCommandsDirectory, (err, files) => {
      if (err) {
        ...
        files.forEach((file) => {
          ...
          if (!cmd || !cmd.run) {
            return this.warn(
              t("DiscordMusicBot.auto_312") +
                file.split(".")[0] +
                t("DiscordMusicBot.auto_313")
            );
          }
          if (cmd.disabled) {
            return;
      ...
      if (err) {
        ...
        files.forEach((file) => {
          ...
          if (!cmd.command || !cmd.run) {
            return this.warn(
              t("DiscordMusicBot.auto_314") +
                file.split(".")[0] +
                t("DiscordMusicBot.auto_315")
            );
          }
  ...
  createController(guild, player) {
    ...
    const row2 = new ActionRowBuilder().addComponents(
      ...
      new ButtonBuilder()
        .setStyle(ButtonStyle.Primary)
        .setCustomId(`controller:${guild}:AutoSettings`)
        .setLabel(this.translateGuild(guild, "Controller.autoMenuButton")),
      new ButtonBuilder()
        .setStyle(ButtonStyle.Success)
        .setCustomId(`controller:${guild}:Save`)
        .setLabel(this.translateGuild(guild, "Controller.saveButton"))
    );
    ...
    if (queueTracks && queueTracks.length > 0) {
      ...
      const selectMenu = new StringSelectMenuBuilder()
        ...
        .setPlaceholder(
          `📋 Hàng đợi (${queueTracks.length} bài) – Chọn để nhảy đến bài`
        )
        .setMaxValues(1)
        .addOptions(options);
